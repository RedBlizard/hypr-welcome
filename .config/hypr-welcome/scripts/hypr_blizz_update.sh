...
folders=("alacritty" "btop" "cava" "dunst" "hypr" "kitty" "Kvantum" "networkmanager-dmenu" "nwg-look" "pacseek" "pipewire" "qt6ct" "ranger" "sddm-config-editor" "systemd" "Thunar" "waybar" "waypaper" "wlogout" "wofi" "xsettingsd" "gtk-2.0" "gtk-3.0" "gtk-4.0" "starship" "swaync" "Code - OSS")
...
for folder in "${folders[@]}"; do
    ...
    if [ "$folder" == "Code - OSS" ]; then
        ...
        if [ -f "$folder_path/User/settings.json" ]; then
            ...
            echo "Backed up VS Code settings.json safely."
        else
...
if [[ "$update_choice" =~ ^[Yy]$ ]]; then
    ...
    # ============================================================
    # SAFE: Sync VS Code Settings (Only settings.json)
    # ============================================================
    show_message "Updating VS Code settings..." "$BLUE"
    if [ -f "$HOME/hyprland-dots/Hyprland-blizz/.config/Code - OSS/User/settings.json" ]; then
        mkdir -p "$HOME/.config/Code - OSS/User"
        cp "$HOME/hyprland-dots/Hyprland-blizz/.config/Code - OSS/User/settings.json" "$HOME/.config/Code - OSS/User/settings.json"
        show_message "VS Code settings.json updated." "$GREEN"
    else
        show_message "VS Code settings.json not found in repo, skipping." "$RED"
    fi
...
if ! check_symlinks; then
    ...
    # This MUST point to the global script in .config/hypr/scripts/, NOT the welcome folder
    global_wallpaper_script="$HOME/.config/hypr/scripts/select-wallpaper"
    symlink="/usr/bin/select-wallpaper"
    sudo ln -sf "$global_wallpaper_script" "$symlink"
fi
