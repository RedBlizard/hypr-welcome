...
selected_config=$(printf "%s\n" "${WAYBAR_CONFIGS[@]}" | yad --list --column="Configuration" --separator="" --no-headers --title="Choose Configuration" --text="" --center --button="   Close me if you dare ..." --width=900 --height=445 --fixed --image="$logo_path")
...
pgrep -x swww-daemon >/dev/null || (awww-daemon & disown)
...
cp "${RANDOMPICS}" "$HOME/.cache/awww/current_wallpaper.png"
